...
impl AuthManagerConfig for TestAuthManagerConfig {
    ...
    fn automatic_account_selection(&self) -> AutomaticAccountSelection {
        self.0.automatic_account_selection
    }
...
async fn auth_manager_file_auth_ignores_environment_and_reapplies_workspace_policy() {
    ...
    let mut api_only_config = build_config(
        codex_home.path(),
        /*forced_login_method*/ None,
        /*forced_chatgpt_workspace_id*/ None,
    )
    .await;
    api_only_config.managed_auth_policy.allowed_login_methods = Some(vec![ForcedLoginMethod::Api]);
    assert!(
        AuthManager::new_from_file_auth(api_only_config)
            .await
            .unwrap()
            .is_none(),
        "file-only auth cannot bypass managed login policy"
    );
    ...
    config.managed_auth_policy.allowed_chatgpt_workspaces =
        Some(vec![WORKSPACE_ID_ALLOWED.to_string()]);
    let manager = AuthManager::new_from_file_auth(config)
        .await
        .unwrap()
        .unwrap();
