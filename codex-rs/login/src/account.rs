...
impl AccountStore {
    ...
    pub fn import_current(
        ...
        let auth = load_auth_dot_json(&self.codex_home, root_store_mode)?
    ...
    pub fn chatgpt_auth_for_account(
        &self,
        account_id: &AccountId,
    ) -> std::io::Result<Option<CodexAuth>> {
        let auth = CodexAuth::from_auth_storage(
            &self.account_home(account_id),
            AuthCredentialsStoreMode::File,
        )?;
        match auth {
            Some(auth) if auth.is_chatgpt_auth() => Ok(Some(auth)),
            Some(_) => Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                "account auth is not a ChatGPT login",
            )),
            None => Ok(None),
        }
    }

    pub fn auth_manager_for_account(&self, account_id: &AccountId) -> Arc<AuthManager> {
        AuthManager::shared(
            self.account_home(account_id),
            false,
            AuthCredentialsStoreMode::File,
    ...
    fn account_home(&self, account_id: &AccountId) -> PathBuf {
...
fn is_managed_chatgpt_auth(auth: &AuthDotJson) -> bool {
    match auth.auth_mode {
        Some(ApiAuthMode::Chatgpt) => true,
        Some(ApiAuthMode::ApiKey | ApiAuthMode::ChatgptAuthTokens) => false,
