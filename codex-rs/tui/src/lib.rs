...
async fn maybe_run_startup_account_picker(
    ...
    let selectable_ids: HashSet<AccountId> = store
    ...
    let candidates: Vec<AccountCandidate> = store
        .candidates()?
        .into_iter()
        .filter(|candidate| candidate.enabled && selectable_ids.contains(&candidate.id))
        .collect();
    ...
    let default_idx = candidates
        .iter()
        .position(|candidate| !candidate.blocked)
        .unwrap_or(0);
    let picker_candidates = candidates
        .iter()
        .enumerate()
        .map(|(idx, candidate)| account_picker_candidate(candidate, idx == default_idx))
        .collect();
